...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                else if(ch == '*') {   // multi-line comment entry point
                    while (ch != EOFCH) {
                        nextCh();
                        if(ch == '*') {
                            nextCh();
        ...
        switch (ch) {
            ...
            case '|':
                ...
                if(ch == '=') {
                    nextCh();
                    return new TokenInfo(BINCOR_ASSIGN,line);
                }
                else if(ch == '|') {
                    nextCh();
                    return new TokenInfo(LOR,line);
                }
                return new TokenInfo(BINCOR,line);
            case '^':
                nextCh();
                if(ch == '=') {
                    nextCh();
                    return new TokenInfo(BEXCOR_ASSIGN,line);
                }
                return new TokenInfo(BEXCOR,line);
            case '\'':
                buffer = new StringBuffer();
                buffer.append('\'');
                nextCh();
    ...
    // Helper method for the scanner to parse Double literals containing .e | .E
    // also handles the case where it ends with d or D
