...
public class Parser {
    ...
    private ArrayList<JMember> interfaceBody() {
        ArrayList<JMember> body = new ArrayList<>();
        mustBe(LCURLY);
        while(!see(RCURLY) && !see(EOF)) {
            ArrayList<String> mods = modifiers();
            if(!mods.contains(ABSTRACT.image()))    // if abstract modifier wasn't made explicit, add it implicitly
                mods.add(ABSTRACT.image());
            JMember member = interfaceMemberDecl(mods);
            body.add(member);
        }
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        ...
        } else if (have(DO)) { // do ... while
            JStatement statement = statement();
            mustBe(WHILE);    // while must be the token after statement
            JExpression parExpression = parExpression();
            mustBe(SEMI);    // semi at the end
            return new JDoStatement(line,statement,parExpression);
        } else if (have(FOR)) {
            // Default to null since forInit, condition, and forUpdate are optional
            ArrayList<JStatement> initList = null;
            ArrayList<JStatement> updateList = null;
            JExpression condition = null;   // default to null if no condition exists

            mustBe(LPAREN); // (
            if(!have(SEMI)) {
                initList = forInit(); // [forInit()]
                mustBe(SEMI);
            }
            if(!have(SEMI)) {
                condition = expression(); // condition of for loop
                mustBe(SEMI);
            }
            if(!have(RPAREN)) {
            ...
            return new JForStatement(line,initList,condition,updateList,body);
        } else if (have(BREAK)) {
            mustBe(SEMI);
            ...
            else { // FINALLY must be present if no catches
                finallyBlock = block();
            }
    ...
    /**
     * Parses and returns a list of statements to initialize a for loop
     *
     * <pre>
     ...
     * </pre>
     * @return
     * List of statement expressions - if it is looking at a local variable declaration
     * List of a single JVariableDeclaration object - if not
     */
    private ArrayList<JStatement> forInit() {
        int line = scanner.token().line();
        ArrayList<JStatement> list = new ArrayList<>();
        // If not looking at a local variable declaration, just gather statement expressions
        if(!seeLocalVariableDeclaration()) {
            do {
                list.add(statementExpression());
            } while (have(COMMA));

        // Else, return a list containing a single JVariableDeclaration object
        } else {
            Type type = type();
            ArrayList<JVariableDeclarator> variableDecList = variableDeclarators(type);
            JVariableDeclaration variableDeclaration = new JVariableDeclaration(line,variableDecList);
    ...
    /**
     * Parses and returns a list of statements in the for loop update
     *
     * <pre>
     *      forUpdate ::= statementExpression {, statementExpression}
     * </pre>
     *
     * @return list of statements in the for loop update
     */
     ...
     *                      | DEFAULT COLON
     * </pre>
     * @return an AST for switch label
     */
     ...
     * </pre>
     * @return an AST for switch block statement group
     */
    ...
    private Type basicType() {
        if (have(BOOLEAN)) {
            ...
            return Type.INT;
        } else if(have(DOUBLE)) { // Parse for double basic type
            return Type.DOUBLE;
        } else if(have(LONG)) {   // Parse for long basic type
            return Type.LONG;
        } else {
     ...
     *                                [ ( ASSIGN | PLUS_ASSIGN | MINUS_ASSIGN | MULT_ASSIGN | MOD_ASSIGN |
     *                                ARSHIFT_ASSIGN | LRSHIFT_ASSIGN | ALSHIFT_ASSIGN | DIV_ASSIGN |
     *                                BEXCOR_ASSIGN | BINCOR_ASSIGN | BAND_ASSIGN) assignmentExpression ]
     * </pre>
     *
     ...
     *                          [TERNIF assignmentExpression TERNELSE conditionalExpression]
     * </pre>
     *
    ...
    private JExpression conditionalExpression() {
        ...
        if (have(TERNIF)) { // Begin parsing tern if else expression
            JExpression thenPart = assignmentExpression();
            mustBe(TERNELSE); // Must be a TERNELSE token
     ...
     *
     * <pre>
     *     inclusiveOrExpression ::== exclusiveOrExpression { BINCOR exclusiveOrExpression }
     * </pre>
     ...
     *
     * <pre>
     *     exclusiveOrExpression ::== andExpression { BEXCOR andExpression }
     * </pre>
     * @return an AST for an exclusive or expression
     */
     ...
     *
     * <pre>
     *   equalityExpression ::= relationalExpression { (EQUAL | NEQUAL) relationalExpression }
     * </pre>
     *
     ...
     *
     * <pre>
     *   multiplicativeExpression ::= unaryExpression { (STAR | DIVIDE | REMAINDER) unaryExpression }
     * </pre>
     *
    ...
    private JExpression literal() {
        ...
        if (have(CHAR_LITERAL)) {
            ...
            return new JLiteralBoolean(line, scanner.previousToken().image());
        } else if (have(LONG_LITERAL)) {  // Parse for Long literal
            return new JLiteralLong(line,scanner.previousToken().image());
        } else if (have(DOUBLE_LITERAL)) { // Parse for Double literal
