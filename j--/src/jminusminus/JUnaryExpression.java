...
class JNegateOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (operand.type() == Type.INT) {
            ...
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for unary negation expression");
        }
        return this;
    }
...
class JUnaryPlusOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if(operand.type() == Type.INT) {
            ...
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for unary plus expression");
        }
        return this;
    }
...
class JPostIncrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            JAST.compilationUnit.reportSemanticError(line, "Operand to ++ (post) must have an LValue.");
            type = Type.ANY;
        } else {
            operand = (JExpression) operand.analyze(context);
            operand.type().mustMatchExpected(line(), Type.INT);
...
class JPreDecrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            JAST.compilationUnit.reportSemanticError(line, "Operand to -- (pre) must have an LValue.");
            type = Type.ANY;
        } else {
            operand = (JExpression) operand.analyze(context);
            operand.type().mustMatchExpected(line(), Type.INT);
